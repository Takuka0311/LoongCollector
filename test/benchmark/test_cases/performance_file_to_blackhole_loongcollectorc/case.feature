...
Feature: performance file to blackhole LoongCollector-C
  ...
  Scenario: PerformanceFileToBlackholeiLoongCollectorC
    ...
    When start monitor {LoongCollectorC}, with timeout {11} min
    When generate random nginx logs to file, speed {15}MB/s, total {10}min, to file {./test_cases/performance_file_to_blackhole_loongcollectorc/a.log}
