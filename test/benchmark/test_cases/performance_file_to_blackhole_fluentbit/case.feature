...
Feature: performance file to blackhole fluentbit
  ...
  Scenario: PerformanceFileToBlackholeFluentbit
    ...
    When start monitor {fluent-bit}, with timeout {11} min
    When generate random nginx logs to file, speed {15}MB/s, total {10}min, to file {./test_cases/performance_file_to_blackhole_fluentbit/a.log}
