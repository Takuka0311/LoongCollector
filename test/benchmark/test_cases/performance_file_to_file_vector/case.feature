...
Feature: performance file to file vector
  ...
  @e2e-performance @docker-compose @vectorfile
  Scenario: PerformanceFileToFileVector
    Given {docker-compose} environment
    Given docker-compose boot type {benchmark}
    When start docker-compose {performance_file_to_file_vector}
    When start monitor {vector}, with timeout {6} min
    When generate random nginx logs to file, speed {10}MB/s, total {5}min, to file {./test_cases/performance_file_to_file_vector/a.log}
