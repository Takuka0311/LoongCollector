...
Feature: performance file to blackhole LoongCollector-CGo
  ...
  Scenario: PerformanceFileToBlackholeLoongCollectorCGo
    ...
    When start monitor {LoongCollectorCGo}, with timeout {11} min
    When generate random nginx logs to file, speed {15}MB/s, total {10}min, to file {./test_cases/performance_file_to_blackhole_loongcollectorcgo/a.log}
